...
class NERPipeline:
    ...
    def compute_metrics(self, eval_pred):
        ...
        # Flatten predictions and labels while ignoring special tokens (label = -100)
        true_predictions = []
        true_labels = []
    ...
    def train(self):
        ...
        # Convert to a pandas DataFrame (with one row)
        metrics_df = pd.DataFrame([metrics])
        
        # Create a filename based on the model name (replace any '/' with '_' to avoid file system issues)
